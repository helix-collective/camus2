...
shutdownLocalProxy :: IOR ()
shutdownLocalProxy = do
  scopeInfo "shutdown local proxy" $ do
    proxyDir <- getProxyDir
    callCommandInDir proxyDir "docker-compose kill"
...
writeProxyDockerCompose tcfg path = T.writeFile path (T.intercalate "\n" lines)
  where
    ...
    ledir = tc_letsencryptPrefixDir tcfg
    lewwwdir = tc_letsencryptWwwDir tcfg
    nginxImage = tc_nginxDockerImage tcfg
