...
To that end, rename the file to docker-compose.yml.tpl so that the tool will recognise it as a template. More detail around the mustache magid in [Template anything](/camus2/userguide/3-reference/3-templateanything) available with the tool after the link.
