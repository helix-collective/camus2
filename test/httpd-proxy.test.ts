...
function makeConfig(setup: TestSetup, mode:"controller"|"target"): ToolConfig {
  return makeToolConfig({
    ...
    deployMode: makeDeployMode("proxy", makeProxyModeConfig({
      ...
      remoteStateS3: (mode==='controller') ?
        makeMaybe<string,"just">("just", `s3://${localstack.bucket}/${localstack.prefix}/${setup.randomstr}/remotestate`) :
        makeMaybe<string,"nothing">("nothing",null),
...
for(const remoteMode of ["local","remote"] as const) {
  describe(`Run release mode ${remoteMode}`, ()=>{
    ...
    test("Config and deployment of a httpd server", async () => {
      ...
      await writeToolConfig(testSetup, makeConfig(testSetup,'controller'),'controller');
      await writeToolConfig(testSetup, makeConfig(testSetup,'target'),'target');
      ...
      if(remoteMode === 'remote') {
        /// In deployment this happens on another machine coordinated by S3 remote state
